...
import { Printer, X, FileText, FileImage, CheckCircle, SlidersHorizontal } from "lucide-react";
...
export function ExportPdfDialog({ open, onOpenChange, items, title = "Peças" }: ExportPdfDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 gap-0" style={{ maxWidth: 1000, width: "95vw", borderRadius: 14, backgroundColor: "#fff", border: "none", boxShadow: "0 24px 48px -12px rgba(28,25,23,0.18)" }}>
        ...
        {/* ── Header escuro ── */}
        <div style={{ padding: "20px 28px 16px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", background: "linear-gradient(135deg, #1c1917 0%, #292524 100%)", borderRadius: "14px 14px 0 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: "#7c3aed", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <Printer style={{ width: 16, height: 16, color: "#fff" }} />
            </div>
            <div>
              <h2 style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.03em", color: "#fff", margin: 0, fontFamily: '"Space Grotesk", sans-serif' }}>Exportar PDF</h2>
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", margin: 0 }}>Defina os filtros e opções antes de gerar o PDF</p>
            </div>
          </div>
        ...
        <div style={{ display: "flex", height: 560, overflow: "hidden" }}>
          ...
          {/* ── Painel esquerdo — opções do PDF + ações ── */}
          <div style={{ width: 260, flexShrink: 0, borderRight: "1px solid #f0ede8", display: "flex", flexDirection: "column", backgroundColor: "#fafaf9" }}>
            <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 8px" }}>

              <p style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.09em", color: "#a8a29e", margin: "0 0 10px" }}>Opções do PDF</p>
              ...
              {groupsInSelection.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  ...
                  <p style={{ fontSize: 10, color: "#a8a29e", margin: "0 0 6px" }}>Grupos juntos numa página. Desmarcados: 1 por página.</p>
                    ...
                    {groupsInSelection.map(g => {
                      ...
                      return (
                        <div key={g.key}
                          ...
                          style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 8px", borderRadius: 7, border: `1px solid ${on ? "#bfdbfe" : "#e7e5e4"}`, backgroundColor: on ? "#f0f9ff" : "#fff", cursor: "pointer" }}>
                          <div style={{ width: 14, height: 14, borderRadius: 3, border: `2px solid ${on ? "#2563eb" : "#d4d4d0"}`, backgroundColor: on ? "#2563eb" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                            {on && <CheckCircle style={{ width: 8, height: 8, color: "#fff" }} />}
                          </div>
                          <span style={{ flex: 1, minWidth: 0, fontSize: 11, fontWeight: 600, color: "#1c1917", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.key}</span>
                          <span style={{ flexShrink: 0, fontSize: 9, fontWeight: 700, color: on ? "#2563eb" : "#78716c" }}>{on ? Math.ceil(g.count / MAX_ITEMS_PER_COMBINED_PAGE) : g.count}p</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              {/* Página divisória por evento */}
              {eventCount > 1 && (
                <div onClick={() => setGroupByEvent(!groupByEvent)}
                  style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, border: `1px solid ${groupByEvent ? "#bfdbfe" : "#e7e5e4"}`, backgroundColor: groupByEvent ? "#f0f9ff" : "#fff", cursor: "pointer", marginBottom: 10 }}>
                  <div style={{ width: 16, height: 16, borderRadius: 4, border: `2px solid ${groupByEvent ? "#2563eb" : "#d4d4d0"}`, backgroundColor: groupByEvent ? "#2563eb" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {groupByEvent && <CheckCircle style={{ width: 9, height: 9, color: "#fff" }} />}
                  </div>
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#1c1917", margin: 0 }}>Divisória por evento</p>
                    <p style={{ fontSize: 9, color: "#a8a29e", margin: 0 }}>{eventCount} eventos com capa própria</p>
                  </div>
                </div>
              )}
              ...
              {selected.some(i => i.bookUrl) && (() => {
                ...
                return (
                  <div style={{ backgroundColor: "#f5f3ff", border: "1px solid #ddd6fe", borderRadius: 8, padding: "8px 10px", marginBottom: 10 }}>
                    <p style={{ fontSize: 11, fontWeight: 800, color: "#6d28d9", margin: 0, display: "flex", alignItems: "center", gap: 4 }}>
                      <FileText style={{ width: 10, height: 10 }} />
                      {all ? "Todas com book" : `Book: ${nBook} de ${selected.length}`}
                    </p>
                    <div style={{ height: 3, borderRadius: 999, backgroundColor: "#ddd6fe", margin: "5px 0", overflow: "hidden" }}>
                    ...
                    <p style={{ fontSize: 10, color: "#7c3aed", margin: 0 }}>
                      {all ? "Abre o PDF do book pronto." : `O book abre pronto; as outras ${selected.length - nBook} saem no PDF.`}
                    </p>
                  </div>
                );
              })()}
            ...
            {/* Rodapé com botões */}
            <div style={{ padding: "12px 16px", borderTop: "1px solid #f0ede8", display: "flex", flexDirection: "column", gap: 7 }}>
              ...
              <button
                ...
                style={{ width: "100%", height: 42, borderRadius: 8, backgroundColor: selected.length === 0 ? "#e7e5e4" : "#7c3aed", border: "none", color: selected.length === 0 ? "#a8a29e" : "#fff", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, cursor: selected.length === 0 ? "not-allowed" : "pointer" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div style={{ padding: "12px 16px", borderBottom: "1px solid #f0ede8", backgroundColor: "#fafaf9", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, flexShrink: 0 }}>
                <SlidersHorizontal style={{ width: 13, height: 13, color: "#a8a29e" }} />
                <span style={{ fontSize: 10, fontWeight: 700, color: "#a8a29e", textTransform: "uppercase", letterSpacing: "0.07em" }}>Filtros</span>
              </div>
              ...
              {hasFilters && (
                <button onClick={clearFilters}
                  style={{ display: "inline-flex", alignItems: "center", gap: 4, height: 30, padding: "0 10px", borderRadius: 6, background: "none", border: "1px solid #e7e5e4", color: "#78716c", cursor: "pointer", fontSize: 11, fontWeight: 600, flexShrink: 0 }}>
            ...
            {/* Cabeçalho da lista */}
            {filtered.length > 0 && (
              <div style={{ padding: "10px 16px 8px", borderBottom: "1px solid #f0ede8", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#78716c" }}>{selected.length} de {filtered.length} {filtered.length === 1 ? "peça" : "peças"}</span>
                  <button onClick={() => setExcludedIds(new Set())} disabled={selected.length === filtered.length}
                    style={{ background: "none", border: "none", padding: 0, fontSize: 11, fontWeight: 700, cursor: selected.length === filtered.length ? "default" : "pointer", color: selected.length === filtered.length ? "#d4d4d0" : "#7c3aed" }}>Todas</button>
                  <span style={{ color: "#e7e5e4" }}>·</span>
                  <button onClick={() => setExcludedIds(new Set(filtered.map(i => i.id)))} disabled={selected.length === 0}
                    style={{ background: "none", border: "none", padding: 0, fontSize: 11, fontWeight: 700, cursor: selected.length === 0 ? "default" : "pointer", color: selected.length === 0 ? "#d4d4d0" : "#7c3aed" }}>Nenhuma</button>
                </div>
                <span style={{ fontSize: 10, color: "#a8a29e" }}>
                  {selected.filter(i => i.approvalThumbUrl).length} com thumb · {selected.filter(i => !i.approvalThumbUrl).length} sem thumb{selected.some(i => i.bookUrl) ? ` · ${selected.filter(i => i.bookUrl).length} com book` : ""}
                </span>
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10 }}>
                <div style={{ width: 56, height: 56, borderRadius: 14, backgroundColor: "#f3f4f3", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <FileText style={{ width: 24, height: 24, color: "#d4d4d0" }} />
                </div>
                <p style={{ fontSize: 13, fontWeight: 600, color: "#a8a29e", margin: 0 }}>Nenhum item encontrado</p>
                {hasFilters && <button onClick={clearFilters} style={{ fontSize: 11, fontWeight: 700, color: "#7c3aed", background: "none", border: "none", cursor: "pointer" }}>Limpar filtros</button>}
              </div>
            ) : (
                ...
                {filtered.map((item: any) => {
                  ...
                  return (
                    <div key={item.id}
                      ...
                      style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 9, border: `1px solid ${picked ? "#f0ede8" : "#e7e5e4"}`, backgroundColor: picked ? "#fff" : "#fafaf9", opacity: picked ? 1 : 0.5, cursor: "pointer", transition: "opacity 0.1s" }}>
                      <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, border: `2px solid ${picked ? "#7c3aed" : "#d4d4d0"}`, backgroundColor: picked ? "#7c3aed" : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {picked && <CheckCircle style={{ width: 9, height: 9, color: "#fff" }} />}
                      </div>
                      <div style={{ width: 44, height: 44, borderRadius: 7, overflow: "hidden", flexShrink: 0, border: "1px solid rgba(0,0,0,0.06)", backgroundColor: "#f3f4f3", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {hasThumb && thumbSrc
                          ...
                          : <FileImage style={{ width: 16, height: 16, color: "#d4d4d0" }} />}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 1 }}>
                          <span style={{ fontSize: 10, fontWeight: 800, color: "#7c3aed", fontFamily: "monospace" }}>{item.displayId}</span>
                          ...
                          {item.bookUrl && (
                            <span title="Coberta por book da Arte" data-testid={`badge-book-export-${item.id}`}
                              style={{ flexShrink: 0, display: "inline-flex", alignItems: "center", gap: 3, fontSize: 9, fontWeight: 800, color: "#6d28d9", backgroundColor: "#f5f3ff", border: "1px solid #ddd6fe", borderRadius: 3, padding: "1px 5px", textTransform: "uppercase", letterSpacing: "0.04em" }}>
                              <FileText style={{ width: 7, height: 7 }} /> Book
                            </span>
                          )}
                        </div>
