...
const TITULO_DA_SECAO: React.CSSProperties = { margin: "0 0 8px", fontSize: FS.small, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: T.apoio };

function Campo({ rotulo, children }: { rotulo: string; children: React.ReactNode }) {
  return (
    <div style={{ minWidth: 0 }}>
      <dt style={{ fontSize: FS.small, fontWeight: 700, color: T.apoio, marginBottom: 2 }}>{rotulo}</dt>
  ...
  return (
    <section data-testid={`detalhe-linha-${linha.id}`} style={{ border: `1px solid ${T.border}`, borderRadius: R.lg, padding: isMobile ? 12 : 16, display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <span style={{ fontSize: FS.small, fontWeight: 800, color: T.apoio, letterSpacing: "0.06em", textTransform: "uppercase" }}>Peça {numero}</span>
      ...
      <dl style={{ margin: 0, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: "12px 20px" }}>
        ...
        <Campo rotulo="Quantidade">
          ...
          {pecas.length > 0 && criadas !== linha.quantidade && (
            <span style={{ display: "block", fontSize: FS.small, color: TOM.alerta.text, fontWeight: 700 }}>criadas {criadas} un.</span>
          )}
        </Campo>
      ...
      {textoDaObservacao(linha.observacao) && (
        <div>
          ...
          <p style={{ margin: 0, padding: "10px 12px", borderLeft: `3px solid ${T.bdark}`, background: T.bg, borderRadius: R.sm, fontSize: 14, color: T.strong, lineHeight: 1.55, whiteSpace: "pre-wrap", overflowWrap: "anywhere" }}>
            “{textoDaObservacao(linha.observacao)}”
          </p>
        </div>
      )}
      ...
      {linha.status === "recusado" && (
        <p style={{ margin: 0, fontSize: 14, color: TOM.perigo.text, lineHeight: 1.5 }}><strong>Motivo da recusa:</strong> {linha.motivoRecusa}</p>
      )}
      {linha.status === "cancelado" && (
        <p style={{ margin: 0, fontSize: 14, color: T.strong, lineHeight: 1.5 }}><strong>Motivo do cancelamento:</strong> {linha.motivoCancelamento ?? "—"}</p>
      )}
        ...
        <div style={{
          ...
          <section data-testid="historico-do-pedido" aria-labelledby="titulo-historico-pedido" style={{ minWidth: 0 }}>
            ...
            {isLoading ? (
              ...
              <p role="alert" style={{ margin: 0, fontSize: FS.body, color: TOM.perigo.text, lineHeight: 1.45 }}>
