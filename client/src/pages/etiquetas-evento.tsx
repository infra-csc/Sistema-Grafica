...
export default function EtiquetasEvento() {
  ...
  if (itensFalharam || eventoFalhou) {
    return (
      <div style={{ padding: 40 }}>
        <p role="alert" data-testid="etiquetas-erro" style={{ margin: 0, fontSize: 14, color: "#b91c1c", fontWeight: 600 }}>
          Não foi possível carregar as peças do evento.
        </p>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
          <button type="button" onClick={() => refetch()} className="etq-foco" style={{ minHeight: 44, padding: "0 16px", borderRadius: 8, border: "1px solid #d6d3d1", background: "#fff", cursor: "pointer", font: "inherit", fontSize: 13, fontWeight: 600, color: "#44403c" }}>
            Tentar de novo
          </button>
          <Link href={voltarHref} className="etq-foco" style={{ display: "inline-flex", alignItems: "center", gap: 6, minHeight: 44, padding: "0 12px", borderRadius: 8, color: "#44403c", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
  ...
  const blocoDeAcao = (
    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", justifyContent: isMobile ? "stretch" : "flex-end", flex: isMobile ? undefined : "1 1 420px", minWidth: 0 }}>
      <div style={{ flex: "1 1 240px", minWidth: 0, textAlign: isMobile ? "left" : "right" }}>
        <p data-testid="resumo-da-impressao" aria-live="polite" style={{ margin: 0, fontSize: fonteBase, fontWeight: 700, lineHeight: 1.35, color: "#1c1917" }}>
          {resumo.texto}
        </p>
        {motivoParado ? (
      ...
      <button type="button" onClick={imprimir} data-testid="button-imprimir-etiquetas" className="etq-foco"
        ...
        style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, minHeight: isMobile ? 48 : 40, padding: "0 18px", borderRadius: 8, border: "none", flex: isMobile ? "1 1 100%" : undefined, backgroundColor: motivoParado ? "#e7e5e4" : "#1c1917", color: motivoParado ? "#57534e" : "#fff", cursor: motivoParado ? "not-allowed" : "pointer", font: "inherit", fontSize: isMobile ? 15 : 13.5, fontWeight: 800 }}>
        <Printer aria-hidden="true" style={{ width: 15, height: 15 }} /> {esperandoLogo ? "Buscando o logo…" : "Imprimir / PDF"}
      </button>
    </div>
  );
  ...
  const painel = (
    <div id="etq-painel" data-testid="painel-de-opcoes" style={{ display: "flex", flexDirection: "column", gap: 12, minWidth: 0 }}>
      <SecaoDeOpcoes titulo="1 · O que imprimir" testid="secao-o-que-imprimir">
        <p data-testid="contagem-marcadas" aria-live="polite" style={{ margin: 0, fontSize: fonteBase, fontWeight: 700, color: "#1c1917" }}>
          {pecas.length} de {pool.length} {pool.length === 1 ? "peça marcada" : "peças marcadas"}
          {vistaEstreita && <span style={{ fontWeight: 600, color: "#57534e" }}> · mostrando {visiveis.length}</span>}
        </p>
          ...
          <>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <label style={{ position: "relative", flex: "1 1 160px", minWidth: 0, display: "block" }}>
                ...
                <Search aria-hidden="true" style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", width: 14, height: 14, color: "#78716c" }} />
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              <button type="button" data-testid="selecao-todas" className="etq-foco" onClick={() => setDesmarcadas(new Set())} style={botaoLeve}>Todas</button>
              <button type="button" data-testid="selecao-nenhuma" className="etq-foco" onClick={() => setDesmarcadas(new Set(pool.map((p) => p.id)))} style={botaoLeve}>Nenhuma</button>
              {vistaEstreita && visiveis.length > 0 && (
                <button type="button" data-testid="selecao-so-visiveis" className="etq-foco" title="Marca as peças que a busca/filtro está mostrando e desmarca todas as outras."
                  onClick={() => { const ver = new Set(visiveis.map((p) => p.id)); setDesmarcadas(new Set(pool.filter((p) => !ver.has(p.id)).map((p) => p.id))); }} style={botaoLeve}>
              ...
              {impressasNoPool > 0 && faltamNoPool > 0 && (
                <button type="button" data-testid="selecao-so-novas" className="etq-foco"
                  onClick={() => setDesmarcadas(new Set(pool.filter((p) => p.labelPrintedAt).map((p) => p.id)))}
                  style={{ ...botaoLeve, border: "1px solid #fdba74", background: "#fff7ed", color: "#9a3412" }}>
            ...
            <ul data-testid="lista-de-pecas" aria-label="Peças do evento" style={{ listStyle: "none", margin: 0, padding: 0, maxHeight: isMobile ? 300 : 280, overflowY: "auto", border: "1px solid #e7e5e4", borderRadius: 8 }}>
              {visiveis.length === 0 && (
                <li data-testid="busca-sem-resultado" style={{ padding: "14px 12px", fontSize: fonteBase, color: "#57534e" }}>
                  Nenhuma peça com esse filtro.{" "}
                  <button type="button" className="etq-foco" onClick={() => { setBusca(""); setFiltroTipo(null); }} style={{ border: "none", background: "none", padding: 0, minHeight: alvo, font: "inherit", fontWeight: 700, color: "#9a3412", textDecoration: "underline", cursor: "pointer" }}>Limpar</button>
                </li>
              )}
              {visiveis.map((p, i) => {
                  ...
                  <li key={p.id} style={{ borderTop: i > 0 ? "1px solid #f0efee" : "none", contentVisibility: "auto", containIntrinsicSize: "auto 48px" } as React.CSSProperties}>
                    <label style={{ display: "flex", alignItems: "center", gap: 10, minHeight: Math.max(alvo, 44), padding: "5px 10px", cursor: "pointer", backgroundColor: marcada ? "#fff7ed" : "#fff" }}
                      ...
                      <span style={{ minWidth: 0, flex: 1 }}>
                        <span style={{ display: "block", fontSize: fonteBase, fontWeight: 600, color: "#1c1917", overflowWrap: "anywhere", lineHeight: 1.25 }}>
      ...
      {tiposNoPool.length > 0 && (
        <SecaoDeOpcoes titulo="2 · Como sai" testid="secao-como-sai"
          ...
          <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 6 }}>
            {tiposNoPool.map(([t, n]) => (
              <li key={t} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: fonteBase, fontWeight: 700, color: "#1c1917", overflowWrap: "anywhere", minWidth: 0, flex: "1 1 120px" }}>
                  {t} <span style={{ fontWeight: 600, color: "#57534e" }}>· {n} {n === 1 ? "peça" : "peças"}</span>
                </span>
          ...
          {tiposNoPool.length > 1 && (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              <button type="button" className="etq-foco" data-testid="como-sai-tudo-lista" onClick={() => setTiposEmLista(new Set(tiposNoPool.map(([t]) => t)))} style={botaoLeve}>Tudo em lista</button>
              <button type="button" className="etq-foco" data-testid="como-sai-tudo-individual" onClick={() => setTiposEmLista(new Set())} style={botaoLeve}>Tudo individual</button>
              {tiposEmLista !== null && (
                <button type="button" className="etq-foco" data-testid="como-sai-padrao" title="Volta ao padrão: só o 2x1 em lista." onClick={() => setTiposEmLista(null)} style={botaoLeve}>Padrão</button>
              )}
            </div>
          )}
        </SecaoDeOpcoes>
      ...
      <SecaoDeOpcoes titulo="3 · Formato" testid="secao-formato">
        {temLista && (
          <div data-testid="formato-da-lista" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <label style={{ display: "flex", flexDirection: "column", gap: 3, fontSize: 12.5, fontWeight: 600, color: "#44403c", flex: "2 1 170px", minWidth: 0 }}>
                Papel da lista
              ...
              <label title="Quantas vezes cada lista sai — colam dos dois lados do volume." style={{ display: "flex", flexDirection: "column", gap: 3, fontSize: 12.5, fontWeight: 600, color: "#44403c", flex: "1 1 80px", minWidth: 0 }}>
                Cópias
        ...
        {todasAsIndividuais.length > 0 && (
          <div data-testid="formato-das-individuais" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 800, color: "#44403c" }}>Etiquetas individuais · A4, duas por folha</p>
            {/* Orientação só existe para a individual — a lista é sempre em pé. */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", fontSize: 12.5, fontWeight: 600, color: "#44403c" }}>
              Folha
      ...
      <SecaoDeOpcoes titulo="4 · Cabeçalho da etiqueta" testid="secao-cabecalho"
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: 3, fontSize: 12.5, fontWeight: 600, color: "#44403c" }}>
  ...
  return (
    <div style={{ backgroundColor: "#fafaf9", minHeight: "100%" }}>
      <style>{`
        ...
        @media screen {
          .etq-folha { border: 1px solid #d6d3d1; border-radius: 10px; box-shadow: 0 6px 18px rgba(0,0,0,0.07); }
      ...
      <div className="etq-acao" data-testid="barra-das-etiquetas" style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", padding: isMobile ? "10px 12px" : "12px 18px", borderBottom: "1px solid #e7e5e4", position: "sticky", top: 0, backgroundColor: "#fafaf9", zIndex: 5 }}>
        <Link href={voltarHref} data-testid="link-voltar-evento" className="etq-foco" style={{ display: "inline-flex", alignItems: "center", gap: 6, minHeight: alvo, padding: "0 12px", borderRadius: 8, border: "1px solid #e7e5e4", color: "#44403c", fontSize: 13, fontWeight: 600, textDecoration: "none", backgroundColor: "#fff" }}>
          <ArrowLeft aria-hidden="true" style={{ width: 14, height: 14 }} /> {veioDaGrafica ? "Voltar à Gráfica" : "Voltar ao evento"}
        </Link>
        <div style={{ minWidth: 0, flex: "1 1 200px" }}>
          <h1 style={{ margin: 0, display: "flex", alignItems: "center", gap: 6, fontSize: isMobile ? 15 : 16, fontWeight: 800, color: "#1c1917", lineHeight: 1.2 }}>
            <Tags aria-hidden="true" style={{ width: 16, height: 16, color: "#c2410c", flexShrink: 0 }} /> Etiquetas do evento
          </h1>
          <p data-testid="nome-do-evento" style={{ margin: "1px 0 0", fontSize: 13, color: "#57534e", overflowWrap: "anywhere" }}>
            {nome || "Evento"}{event?.truckDepartureDate ? ` · saída ${dataBR(event.truckDepartureDate)}` : ""}
          </p>
        </div>
        {isMobile ? (
          <button type="button" className="etq-foco" data-testid="abrir-opcoes" aria-expanded={opcoesAbertas} aria-controls="etq-painel-movel" onClick={() => setOpcoesAbertas((v) => !v)}
            style={{ display: "inline-flex", alignItems: "center", gap: 6, minHeight: 44, padding: "0 14px", borderRadius: 8, border: "1px solid #d6d3d1", backgroundColor: opcoesAbertas ? "#1c1917" : "#fff", color: opcoesAbertas ? "#fff" : "#1c1917", font: "inherit", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
            <SlidersHorizontal aria-hidden="true" style={{ width: 15, height: 15 }} /> {opcoesAbertas ? "Ver a prévia" : "Opções"}
          </button>
        ) : blocoDeAcao}
      </div>
      ...
      <div className="etq-corpo" style={{ display: isMobile ? "block" : "grid", gridTemplateColumns: isMobile ? undefined : "minmax(320px, 380px) minmax(0, 1fr)", alignItems: "start", gap: 0 }}>
        ...
        <div ref={refDaPrevia} className="etq-previa" data-testid="previa-das-etiquetas"
          ...
          {folhas === 0 && (
            <div className="etq-acao" data-testid="etiquetas-vazio" style={{ maxWidth: 520, margin: "24px auto", padding: "22px 20px", border: "1px dashed #d6d3d1", borderRadius: 12, backgroundColor: "#fff", textAlign: "center" }}>
              <p style={{ margin: 0, fontSize: 15, fontWeight: 800, color: "#1c1917" }}>
                {pool.length > 0 ? "Nenhuma peça marcada" : incluirTodas ? "Este evento não tem peças para etiquetar" : "Nenhuma peça conferida ainda"}
              </p>
              <p style={{ margin: "6px 0 14px", fontSize: 13.5, lineHeight: 1.45, color: "#57534e" }}>
                {pool.length > 0
              ...
              {pool.length > 0 ? (
                <button type="button" className="etq-foco" data-testid="vazio-marcar-todas" onClick={() => setDesmarcadas(new Set())} style={{ ...botaoLeve, minHeight: 44 }}>Marcar todas as {pool.length}</button>
              ) : !incluirTodas ? (
                <button type="button" className="etq-foco" data-testid="vazio-incluir-todas" onClick={() => setIncluirTodas(true)} style={{ ...botaoLeve, minHeight: 44 }}>Incluir as não conferidas</button>
              ) : null}
            </div>
          )}
