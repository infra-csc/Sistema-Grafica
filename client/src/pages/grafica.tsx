...
// ─── Dropdown de filtro de eventos redesenhado ───────────────────────────────
interface EventOption { value: string; label: string; count: number; dotColor?: string }

function EventFilterDropdown({
  value, onChange, options,
}: { value: string; onChange: (v: string) => void; options: EventOption[] }) {
  ...
  const triggerLabel = selected ? selected.label : "Todos os Eventos";
    ...
    <div ref={ref} style={{ position: "relative", flexShrink: 0 }}>
      ...
      <button
        ...
        style={{
          ...
          backgroundColor: open ? "#FFF7ED" : isFiltered ? "#FFF7ED" : "#e8e8e7",
      ...
      >
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", maxWidth: 150 }}>{triggerLabel}</span>
        {isFiltered && (
          <span
            role="button"
      ...
      {open && (
        <div style={{
          ...
          minWidth: 260, maxWidth: 320, overflow: "hidden",
        }}>
          {/* Search */}
          ...
          <div style={{
            ...
            {!search.trim() && (
              ...
              >
                <span style={{ flex: 1 }}>Todos os Eventos</span>
...
export default function Grafica() {
  ...
  const stats = {
    ...
    conferidos:   (itemsForEvent as any[]).filter(i => i.status === 'conferred').length,
