...
export default function ConfigurarCotas() {
  ...
  const { data: rules = [], isLoading: rulesLoading } = useQuery<EventQuotaRule[]>({
    ...
    queryFn: () => fetch(`/api/events/${selectedEventId}/quota-rules`).then(r => r.json()),
    enabled: !!selectedEventId,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ quota, itemTypes }: { quota: string; itemTypes: string[] }) => {
      const res = await apiRequest("PUT", `/api/events/${selectedEventId}/quota-rules`, { quota, itemTypes });
      return res.json();
    },
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ["/api/events", selectedEventId, "quota-rules"] });
  ...
  const selectAll = (quota: string) => {
    setMatrix(prev => ({ ...prev, [quota]: new Set(itemTypes) }));
    setDirty(prev => new Set(prev).add(quota));
  };
    ...
    <div style={{ backgroundColor: T.bg, height: "100%", overflowY: "auto", padding: "36px 40px 80px" }}>
      ...
      <div style={{ marginBottom: 36 }}>
        ...
        <h1 style={{ fontSize: 42, fontWeight: 900, color: T.text, margin: "6px 0 0", fontFamily: "'Space Grotesk', sans-serif", letterSpacing: "-0.04em", lineHeight: 1 }}>
          Configurar Cotas por Evento
        </h1>
        <p style={{ fontSize: 13, color: T.second, margin: "10px 0 0", maxWidth: 560 }}>
          Defina quais tipos de peça cada cota de patrocinador recebe por evento. Estas regras são usadas no Auto-vincular da tela de Vincular Patrocinadores.
        </p>
      </div>

      {/* ── Event selector ── */}
      <div style={{ backgroundColor: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, padding: "20px 24px", marginBottom: 24, display: "flex", alignItems: "center", gap: 16 }}>
        <Settings2 style={{ width: 18, height: 18, color: T.accent, flexShrink: 0 }} />
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 10, fontWeight: 900, color: T.muted, textTransform: "uppercase", letterSpacing: "0.14em", margin: "0 0 6px" }}>Evento</p>
          <div style={{ position: "relative" }}>
            <button
              data-testid="select-event"
              onClick={() => setEventOpen(o => !o)}
              style={{ width: "100%", maxWidth: 480, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", backgroundColor: "#f0efee", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600, color: selectedEvent ? T.text : T.muted, textAlign: "left" }}
            >
              {selectedEvent ? selectedEvent.name : "Selecione um evento..."}
              <ChevronDown style={{ width: 14, height: 14, color: T.muted }} />
            </button>
            {eventOpen && (
              <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, width: 480, backgroundColor: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 50, maxHeight: 280, overflowY: "auto" }}>
                {events.map(ev => (
                  <button
                    key={ev.id}
                    onClick={() => { setSelectedEventId(ev.id); setEventOpen(false); }}
                    style={{ width: "100%", textAlign: "left", padding: "10px 14px", border: "none", background: ev.id === selectedEventId ? "#fff7ed" : "transparent", cursor: "pointer", fontSize: 13, fontWeight: ev.id === selectedEventId ? 700 : 500, color: T.text, display: "flex", alignItems: "center", justifyContent: "space-between" }}
                  >
                    {ev.name}
                    {ev.id === selectedEventId && <Check style={{ width: 14, height: 14, color: T.accent }} />}
                  </button>
                ))}
              </div>
            )}
      ...
      {/* ── No event selected ── */}
      {!selectedEventId && (
        <div style={{ backgroundColor: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, padding: "60px 0", textAlign: "center" }}>
          <Info style={{ width: 40, height: 40, color: T.muted, margin: "0 auto 12px" }} />
          <p style={{ fontSize: 14, fontWeight: 700, color: T.second, margin: "0 0 6px" }}>Selecione um evento para começar</p>
          <p style={{ fontSize: 12, color: T.muted, margin: 0 }}>As regras de cota são configuradas por evento</p>
        </div>
      )}
      ...
      {selectedEventId && (
        <div style={{ backgroundColor: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
              ...
              return (
                <div key={q.key} style={{ padding: "12px 10px", borderLeft: `1px solid ${T.border}`, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                    ...
                    <span style={{ fontSize: 10, color: T.muted, fontWeight: 600 }}>{count} tipo{count !== 1 ? "s" : ""}</span>
                  </div>
          ...
          {/* Rows: item types */}
          {rulesLoading ? (
            <div style={{ padding: "48px 0", textAlign: "center", fontSize: 13, color: T.muted }}>Carregando regras...</div>
          ) : itemTypes.length === 0 ? (
            <div style={{ padding: "48px 0", textAlign: "center", fontSize: 13, color: T.muted }}>
              Nenhum tipo de peça cadastrado em Modelos
            </div>
          ) : (
            itemTypes.map((type, idx) => (
              <div
                key={type}
                style={{ display: "grid", gridTemplateColumns: `220px repeat(${QUOTAS.length}, 1fr)`, borderBottom: idx < itemTypes.length - 1 ? `1px solid ${T.low}` : "none" }}
              >
                <div style={{ padding: "12px 20px", display: "flex", alignItems: "center" }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: T.text, fontFamily: "'Space Grotesk', sans-serif" }}>{type}</span>
                </div>
                {QUOTAS.map(q => {
                  const checked = matrix[q.key]?.has(type) ?? false;
                  return (
                    <div
                      key={q.key}
                      onClick={() => toggleCell(q.key, type)}
                      data-testid={`cell-${q.key}-${type}`}
          ...
          {/* Footer summary */}
          {selectedEventId && itemTypes.length > 0 && (
            <div style={{ padding: "12px 20px", backgroundColor: T.low, borderTop: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontSize: 11, color: T.muted, fontWeight: 600 }}>
                {itemTypes.length} tipo{itemTypes.length !== 1 ? "s" : ""} de peça · Clique nas células para ativar/desativar
              </span>
              {dirty.size > 0 && (
